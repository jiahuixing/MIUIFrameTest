...
public class Test_00000001_FMTimeLineTest extends InstrumentationTestCase {
	...
	public void test_FMTimeLineTest() throws Exception {
		for (int i = 0; i < Lib_Frame_Constants.TEST_LOOPS; i++) {
			...
			stationList = marmot.getUiObject(By.clazz(
					"android.widget.ImageButton").res(
					"com.miui.fm:id/btn_stations_list"));
			testStep += 1;
			...
			newStation = marmot.getUiObject(By.clazz("android.widget.Button")
					.text("新建电台"));
			...
			quit = marmot.getUiObject(By.clazz("android.widget.TextView").text(
					"退出"));
			testStep += 1;
			// marmot.log("15. quit fm.");
