...
public class Test_00000001_BackupTimeLineTest extends InstrumentationTestCase {
    ...
    public void test_BackupTimeLineTest() throws Exception {
        for (int i = 0; i < Lib_Frame_Constants.TEST_LOOPS; i++) {
            ...
            // UiObject2 autoBackup, autoBackupSwitch, autoBackupChoose,
            // autoBackupAppsChooseChecker, autoBackupAppsChoose, confirm;
            UiObject2 autoBackup, autoBackupSwitch, autoBackupChoose, confirm;
            ...
            marmot.waitFor(2);
            // testStep += 1;
            // marmot.log(String.format("%s. auto backup app choose.",
            // testStep));
            ...
            // marmot.waitFor(2);
            // confirm =
            // marmot.getUiObject(By.clazz("android.widget.Button").text(
            // "确定"));
            // confirm.click();
            // marmot.waitFor(2);
            ...
            backup = marmot.getUiObject(By.clazz("android.widget.TextView")
                    .text("本地备份"));
            backup.click();
            marmot.waitFor(2);
            ...
            noBackupSummary = marmot.getUiObject(By.clazz(
                    "android.widget.TextView").text("备份你手机中的数据，设置和应用"));
            if (noBackupSummary == null) {
                testStep += 1;
                marmot.log(String.format("%s. delete backup files.", testStep));
                backupList = marmot.getUiObject(By
                ...
                UiObject2 selectAll, delete;
                // UiObject2 selectAll, delete, alertDialog;
                selectAll = marmot.getUiObject(By
                        .clazz("android.widget.Button").text("全选"));
                if (selectAll != null) {
                    selectAll.click();
                    marmot.waitFor(2);
                }
                delete = marmot.getUiObject(By.clazz("android.widget.Button")
                        .text("删除"));
                delete.click();
                marmot.waitFor(2);
            ...
            enableEncrypt = marmot.getUiObject(By
                    .clazz("android.widget.Button").res(
                            "com.miui.backup:id/enable_encrypt"));
            ...
            marmot.waitFor(2);
            testStep += 1;
            marmot.log(String.format(
                    "%s. apps checkbox and contacts checkbox.", testStep));
            backupApps = marmot.getUiObject(By.clazz("android.widget.TextView")
                    .text("软件程序"));
            backupContacts = marmot.getUiObject(By.clazz(
                    "android.widget.TextView").text("帐号和通讯录"));
            checkBoxApps = backupApps.getParent().getParent().getParent()
                    .findObject(By.res("com.miui.backup:id/data_item_cb"));
            checkBoxContacts = backupContacts.getParent().getParent()
                    .getParent()
                    .findObject(By.res("com.miui.backup:id/data_item_cb"));
            ...
            while (true) {
                cancel = marmot.getUiObject(By.clazz("android.widget.Button")
                        .text("取消"));
                finish = marmot.getUiObject(By.clazz("android.widget.Button")
                        .text("完成"));
            ...
            if (noBackupSummary == null) {
                ...
                if (backupList != null) {
                    ...
                    marmot.waitFor(2);
                    testStep += 1;
                    marmot.log(String.format("%s. start restore backup.",
                            testStep));
            ...
            testStep += 1;
            marmot.log(String.format("%s. quit backup.", testStep));
