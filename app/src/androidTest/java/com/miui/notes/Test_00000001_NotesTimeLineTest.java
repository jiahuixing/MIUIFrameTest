    ...
    public void test_NotesTimeLineTest() throws Exception {
        for (int i = 0; i < Lib_Frame_Constants.TEST_LOOPS; i++) {
            ...
            if (richEditor != null) {
                ...
                while (true) {
                    newNote = marmot.getUiObject(By.clazz(
                            "android.widget.Button").text("新建便签"));
                    if (newNote == null) {
                        marmot.pressBack();
                        marmot.waitFor(1);
                    } else {
                ...
                marmot.waitFor(1);
                testStep += 1;
                marmot.log(String.format("%s. delete note.", testStep));
                UiObject2 notesList, firstNote, delete;
                notesList = marmot.getUiObject(By
                        .clazz("android.widget.ListView"));
