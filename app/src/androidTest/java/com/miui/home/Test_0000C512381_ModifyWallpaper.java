    ...
    public void test_00000001_ModifyWallpaper() throws Exception {
        ...
        UiObject view = this.mm.getUiDevice().findObject(
                new UiSelector().className("android.view.View").resourceId(
                        "com.miui.home:id/wallpaper_thumbnail_view"));
        for (int i = 0; i < 4; i++) {
            view.swipeLeft(4);
            mm.waitFor(2);
        }
        /*
         * if(DisplayWidth == 1440){ for(int i=0;i<4;i++){ mm.move(1000, 2300,
         * 500, 2300);
         * 
         * mm.waitFor(2); } } if(DisplayWidth == 1080){ for(int i=0;i<4;i++){
         * mm.move(900, 1700, 200, 1700); mm.waitFor(2); } }
         */
        mm.click(By.text("其他"));
        mm.waitFor(1);
